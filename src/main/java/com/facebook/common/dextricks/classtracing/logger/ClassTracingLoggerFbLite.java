...
public class ClassTracingLoggerFbLite {
    ...
    public static void classLoaded(Class cls) throws NoSuchAlgorithmException {
        if (sEnabled) {
    ...
    public static void logClassNameInTrace(Class cls) throws NoSuchAlgorithmException {
        ...
        boolean zStartsWith = name.startsWith("X.");
        if (simpleName.length() > 4 || !zStartsWith) {
            try {
                MessageDigest messageDigest = MessageDigest.getInstance("MD5");
                messageDigest.update(name.getBytes());
                byte[] bArrDigest = messageDigest.digest();
                int length = bArrDigest.length;
                bArrCopyOfRange = Arrays.copyOfRange(bArrDigest, length - 4, length);
                i = ENCODED_LONG_CLASS_NAMES_TAG;
            } catch (NoSuchAlgorithmException unused) {
                return;
            }
        } else {
            bArrCopyOfRange = simpleName.getBytes();
